...
def launch_gui():
    ...
    def on_confidence_word_click(event):
        ...
        if word in confidence_index:
            tag = confidence_index[word]

            transcription_notebook.select(transcription_tab_frame)
            transcription_text_widget.see(tag)

            # Effacer tout ancien highlight
            transcription_text_widget.tag_remove("highlight", "1.0", tk.END)

            # Ajouter un highlight temporaire pour édition ou focus
            transcription_text_widget.tag_add("highlight", tag)
