...
# def render_tagged_transcription():
    ...
    # try:
        # Brint("[TAG FORMATTER] 🚀 Démarrage du formattage des balises dans le texte complet")
        # tagged_text_widget.delete("1.0", tk.END)

        # # Lire le texte actuel du widget d’édition
        # full_text = transcription_text_widget.get("1.0", tk.END).strip()
        # words = full_text.split()
        # Brint(f"[TAG FORMATTER] Texte récupéré : {len(words)} mots")
        ...
        # while i < len(words):
            ...
            # if word_lower == "tango":
                # Brint(f"[TAG FORMATTER] 🎯 Détection de TANGO à l’index {i}")
                # tagged_text_widget.insert(tk.END, word + " ", "gray")

                # # Check s’il y a un action keyword juste après
                # if i + 1 < len(words):
                    # next_word = words[i + 1]
                    # next_word_lower = next_word.lower()
                    # if next_word_lower in action_keywords:
                        # color = action_keywords[next_word_lower]
                        # tagged_text_widget.insert(tk.END, f"[{next_word.upper()}] ", color)
                        # i += 1
                    # else:
                        # tagged_text_widget.insert(tk.END, next_word + " ", "black")
                        # i += 1

                # # Check s’il y a un target keyword ensuite
                # if i + 1 < len(words):
                    # next_word = words[i + 1]
                    # next_word_lower = next_word.lower()
                    # if next_word_lower in target_keywords:
                        # color = target_keywords[next_word_lower]
                        # tagged_text_widget.insert(tk.END, f"[{next_word.upper()}] ", color)
                        # i += 1
                    # else:
                        # tagged_text_widget.insert(tk.END, next_word + " ", "black")
        ...
        # # Config des tags de couleurs
        # tagged_text_widget.tag_config("gray", foreground="gray")
        ...
        # Brint("[TAG FORMATTER] ✅ Formattage terminé.")

    # except Exception as e:
        # Brint("[TAG FORMATTER] ❌ ERREUR :", str(e))
def render_tagged_transcription():
    try:
        Brint("[TAG FORMATTER] 🚀 Formattage global avec respect des sauts de ligne")
        tagged_text_widget.delete("1.0", tk.END)
        ...
        Brint("[TAG FORMATTER] ✅ Formattage terminé avec structure respectée.")
